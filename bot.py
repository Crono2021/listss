...
GROUP_ID = -1000000000000   # ← cambia por el ID del grupo donde se crean fichas
TOPIC_ID = 123456           # ← cambia por el tema donde publicar fichas
...
# ==============================
# CREAR FICHA DE PELÍCULA
# ==============================

async def create_ficha_for_movie(title, url, context):
...
async def add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        await msg.reply_text("Uso: /add TITULO (AÑO) URL")
        return
    ...
    for e in data["entries"][letra]:
        if normalize(e["title"]) == new_norm:
            await msg.reply_text("⚠️ Esa película ya existe. No se añadirá ni tendrá ficha.")
            return
    ...
    await msg.reply_text("Película añadida.")


# ==============================
# COMANDO /REMOVE y /DELETE
# ==============================
...
async def remove(update, context):
    ...
    data["entries"][letra] = [
        e for e in data["entries"][letra]
        if normalize(e["title"]) != normalize(titulo)
    ]
...
async def importar(update, context):
    ...
    await update.message.reply_text(f"Modo importación activado para {letra}. Envíame los bloques.")
...
# ==============================
# CAPTURAR TEXTO IMPORTADO
# ==============================

async def texto_import(update, context):
...
def main():
    ...
    # Captura de texto durante importación
